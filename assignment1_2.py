...
col1 = mat_movie_features.getcol(0).toarray()
...
# Get ALL ratings for movies that user has rated
#rated_item_u = 1
#%%
...
#%%
# Experimental stuff (Dont run)
def get_n_largest_idx2(vec, n=1):
    idxs = np.argpartition(vec, -n)[-n:]
    idxs = idxs[np.argsort(vec[idxs])]
    return idxs

p_vec = mat_movie_features.getcol(1).toarray().ravel()  
i_vec = mat_movie_features.getcol(0).toarray().ravel()
